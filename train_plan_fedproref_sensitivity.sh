...
# Training plan: FedProRef parameter sensitivity around default synthesis config
# dataset: cifar100
# alpha: 0.01
# total clients: 10, selected clients: 10, local epochs: 10, seeds: 42 43 44
# method: fedproref
# refiner: mlp
# default point M=100, sigma=0.05 is already available and is intentionally skipped
# added points:
#   M=50,  sigma=0.05
#   M=200, sigma=0.05
#   M=100, sigma=0.01
...
#
# Usage:
#   bash train_plan_fedproref_sensitivity.sh
#   DRY_RUN=1 bash train_plan_fedproref_sensitivity.sh
#   MAX_JOBS=3 bash train_plan_fedproref_sensitivity.sh
...
SENSITIVITY_CONFIGS=(
  "M50_sigma005 50 0.05"
  "M200_sigma005 200 0.05"
  "M100_sigma001 100 0.01"
  "M100_sigma010 100 0.10"
)
...
run_one() {
  ...
  local config_name="$6"
  local aug_gen_per_class="$7"
  local proposal_sigma="$8"
  local exp_name="plan_${method}_${dataset}_a${alpha}_tc${TOTAL_CLIENTS}_sc${SELECT_CLIENTS}_le${LOCAL_EPOCHS}_ps${PARTITION_SEED}_s${seed}_ref${refiner_type}_sens_${config_name}"
  ...
  printf '\n============================================================\n'
  printf 'Dataset=%s alpha=%s method=%s total-client=%s select-client=%s local-epoch=%s seed=%s refiner=%s config=%s M=%s sigma=%s\n' \
    "$dataset" "$alpha" "$method" "$TOTAL_CLIENTS" "$SELECT_CLIENTS" "$LOCAL_EPOCHS" "$seed" "$refiner_type" "$config_name" "$aug_gen_per_class" "$proposal_sigma"
  printf 'Command:'
  printf ' %q' "${cmd[@]}"
  printf '\n============================================================\n'
      ...
      for method in "${METHODS_ARRAY[@]}"; do
        ...
        for refiner_type in "${refiner_types[@]}"; do
          for config in "${SENSITIVITY_CONFIGS[@]}"; do
            read -r config_name aug_gen_per_class proposal_sigma <<< "$config"
            if [[ "${DRY_RUN:-0}" == "1" ]]; then
              run_one "$dataset" "$alpha" "$seed" "$method" "$refiner_type" "$config_name" "$aug_gen_per_class" "$proposal_sigma"
            else
              wait_for_slot
              run_one "$dataset" "$alpha" "$seed" "$method" "$refiner_type" "$config_name" "$aug_gen_per_class" "$proposal_sigma" &
              running_jobs=$((running_jobs + 1))
