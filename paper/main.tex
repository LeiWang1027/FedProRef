...
\IEEEauthorblockA{Paper under preparation}
}
...
\begin{IEEEkeywords}
Federated learning, label skew, feature augmentation, prototype aggregation, pretrained models, communication efficiency.
\end{IEEEkeywords}
...
The goal is to learn a global head $\theta$ without sharing raw images or per-sample features. Under extreme Dirichlet label skew, many clients have $n_{k}^{c}=0$ or very small $n_{k}^{c}$ for class $c$. \method{} addresses this by providing local synthetic features for weak classes using global prototype information.
...
\end{equation}
For $d=512$, the ratio is $\frac{Cd^2}{C(d+1)} \approx d \approx 512\times$. On CIFAR-100, \method{} requires roughly $100\times(512+1)\times4\approx0.2$ MB per client, while covariance transmission requires $100\times512^2\times4\approx100$ MB per client.
...
The Dirichlet concentration parameter controls the severity of label skew. Smaller $\alpha$ values create sharper class imbalance across clients. At $\alpha = 0.01$, clients can miss a large fraction of classes, which directly stresses the classifier-head rows for absent labels. At $\alpha \ge 0.3$, clients observe broader label support, and the problem becomes closer to ordinary distribution mismatch. We therefore separate severe and mild regimes in the analysis.
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{figures/fig_alpha_trend_c100_tiny.pdf}
  \caption{Accuracy trends under varying Dirichlet label skew on CIFAR-100 and Tiny-ImageNet. Values are best global test accuracy within 100 communication rounds. Error bars denote standard deviation over three seeds when available.}
  \label{fig:alpha_trend}
\end{figure*}

Figure~\ref{fig:alpha_trend} reports the full Dirichlet-$\alpha$ trend on CIFAR-100 and Tiny-ImageNet. FedProRef remains consistently strong across severe and moderate label-skew settings, indicating that its advantage is not limited to the most extreme $\alpha$ values.

\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{figures/fig_best_so_far_alpha001_c100_tiny.pdf}
  \caption{Best-so-far accuracy curves under severe label skew on CIFAR-100 and Tiny-ImageNet with $\alpha=0.01$. Curves show the mean best global test accuracy achieved up to each communication round over three seeds.}
  \label{fig:best_so_far}
\end{figure*}

Figure~\ref{fig:best_so_far} shows the best-so-far accuracy trajectory under severe label skew. FedProRef reaches higher accuracy early and maintains a clear advantage throughout training, which is consistent with the last-five-round robustness summary.
