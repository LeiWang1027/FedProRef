...
# Training plan: FedProRef refiner=mlp without prototype similarity merge ablation
...
#
# Usage:
#   bash train_plan_fedproref_refmlp_nosim.sh
#   DRY_RUN=1 bash train_plan_fedproref_refmlp_nosim.sh
#   MAX_JOBS=3 bash train_plan_fedproref_refmlp_nosim.sh
...
run_one() {
  ...
  local exp_name="plan_${method}_${dataset}_a${alpha}_tc${TOTAL_CLIENTS}_sc${SELECT_CLIENTS}_le${LOCAL_EPOCHS}_ps${PARTITION_SEED}_s${seed}_ref${refiner_type}_nosim"
